    ...
    <table className="w-full min-w-[1100px] text-left">
      <thead>
        <tr className="border-b border-slate-200 bg-slate-50">
          <th className="px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-400">Candidate</th>
          ...
          >
            ...
            <td className="px-5 py-4 align-top">
              <div className="flex items-start gap-2">
                ...
                <span className={`text-sm font-semibold ${isPast ? "text-slate-400 line-through decoration-slate-300" : "text-slate-700"}`}>
            ...
            {/* Mode */}
            <td className="px-5 py-4 align-top">
              <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-bold ${isPast ? "border-slate-200 bg-slate-50 text-slate-400" : "border-indigo-100 bg-indigo-50 text-indigo-700"}`}>
            ...
            <td className="px-5 py-4 align-top">
              {interview.location_or_link ? (
                isValidUrl(interview.location_or_link) ? (
                  isPast ? (
                    // Expired — show link as plain text, no join button
...
const CompanyInterviews = () => {
  ...
  const fetchInterviews = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get("/interviews/company-interviews");
      ...
      setToastType("error");
      setToastMsg("Failed to load scheduled interviews.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full">
      <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 max-w-3xl text-sm font-medium leading-6 text-slate-500">
                    View all upcoming interviews you have scheduled across your job openings.
                  </p>
                </div>
              </div>
        ...
        {/* ── Upcoming Interviews ── */}
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 bg-slate-50/80 px-5 py-4 sm:px-6 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                {loading ? "Loading..." : `${upcoming.length} upcoming interview${upcoming.length !== 1 ? "s" : ""} found.`}
              </p>
            </div>
          </div>
          ...
          {loading ? (
            <div className="p-8">
              <Loader message="Loading scheduled interviews..." />
            </div>
          ) : upcoming.length === 0 ? (
            <div className="p-8">
              <EmptyState
                title="No Upcoming Interviews"
                description="There are no upcoming interviews. Go to Applicants to schedule interviews for candidates."
                icon={Calendar}
              />
            </div>
          ) : (
